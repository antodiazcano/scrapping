...
class HouseScrapping:
    ...
    def _get_number_of_photos(self) -> int:
        ...
        photos = self.soup.find_all(
            "div",
            {
                "class": "placeholder-multimedia image overlay-box horizontal printable mb-small"
            },
        )
        hidden_photos = self.soup.find_all(
            "div",
            {
                "class": "placeholder-multimedia image overlay-box horizontal mb-small hide_multimedia"
            },
        )
        photos = [str(photo) for photo in photos if "plano" not in str(photo).lower()]
        hidden_photos = [
            str(hidden_photo)
            for hidden_photo in hidden_photos
            if "plano" not in str(hidden_photo).lower()
        ]

        return len(photos) + len(hidden_photos)
...
"""url = "https://www.idealista.com/inmueble/103896451/"
